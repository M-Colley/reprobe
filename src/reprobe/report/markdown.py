...
def render(r: Report) -> str:
    ...
    for s in r.steps:
        ...
        if adv:
            L.append(f"- **LLM diagnosis** _(advisory, {r.llm.get('model', 'local LLM')})_: {adv.get('likely_cause', '')}")
            for fix in adv.get("suggested_fixes", []):
                L.append(f"  - 💡 {fix}")
        if s.diagnostics.get("log_tail"):
            L.append("- **Log tail:**")
            L.append("  ```")
            for line in str(s.diagnostics["log_tail"]).splitlines()[-15:]:
                L.append(f"  {line}")
            L.append("  ```")
        L.append("")
    ...
    if r.llm.get("summary"):
        ...
        L.append(f"*— generated by {r.llm.get('model','local LLM')}; advisory only, not a verified fact._")
        L.append("")
