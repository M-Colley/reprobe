...
**Guardrails:** `reprobe doctor` self-checks the whole config, asserts the base images are present (`reprobe pull` fetches the published ones), Ollama is reachable, and — with `--golden` — that golden reports over the bundled fixtures still match (`python -m reprobe.golden --update` regenerates them after an intentional change) — so a chair confirms "it still works" in one command before each review window. Everything is digest-pinned where possible, so an old report reproduces years later. GameCI/repo2docker are treated as **best-effort upstreams** behind our interface: a break degrades one runner/strategy with a clean error, not the harness (the specific labels `no-matching-editor-image` / `repo2docker-build-failed` land with the Phase-2/3 runners).
